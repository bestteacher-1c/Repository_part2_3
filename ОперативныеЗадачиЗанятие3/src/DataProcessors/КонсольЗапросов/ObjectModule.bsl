...
Функция ПоместитьНастройкиВоВременноеХранилище(Объект) Экспорт
	...
	ПараметрыПередачи.Вставить("ВыводитьВРезультатахЗапросаЗначенияСсылок",
...
//Сизов +
	...
	ПараметрыПередачи.Вставить("ЦветФонаЗапроса", Объект.ЦветФонаЗапроса);
...
Функция ЗаписатьЗапросыВФайлXML(Знач Объект) Экспорт
	...
	ФайлXML.ЗаписатьНачалоЭлемента("Font");
	ФайлXML.ЗаписатьТекст( XMLСтрока(ЗначениеВСтрокуВнутр(ШрифтЗапрос)));
	ФайлXML.ЗаписатьКонецЭлемента();

	ФайлXML.ЗаписатьНачалоЭлемента("Colour");
	ФайлXML.ЗаписатьТекст( XMLСтрока(ЗначениеВСтрокуВнутр(ЦветШрифт)));
	ФайлXML.ЗаписатьКонецЭлемента();

	ФайлXML.ЗаписатьНачалоЭлемента("Decoration");
	ФайлXML.ЗаписатьТекст( XMLСтрока(ЗначениеВСтрокуВнутр(ЦветФонаЗапроса)));
	ФайлXML.ЗаписатьКонецЭлемента();
		...
		Пока ФайлXML.Прочитать() Цикл 
			...
			Если ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ФайлXML.Имя = "query" Тогда
				...
				//Сизов+
						...
						Если СтрНайти(ДваСимволаЛево, "0") <> 0 Или СтрНайти(ДваСимволаЛево, "1") <> 0 Или СтрНайти(
							ДваСимволаЛево, "2") <> 0 Или СтрНайти(ДваСимволаЛево, "3") <> 0 Или СтрНайти(
							ДваСимволаЛево, "4") <> 0 Или СтрНайти(ДваСимволаЛево, "5") <> 0 Или СтрНайти(
							ДваСимволаЛево, "6") <> 0 Или СтрНайти(ДваСимволаЛево, "7") <> 0 Или СтрНайти(
							ДваСимволаЛево, "8") <> 0 Или СтрНайти(ДваСимволаЛево, "9") <> 0 Тогда
						...
						Пока ФайлXML.Прочитать() Цикл
							...
							Если ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ФайлXML.Имя = "parameter" Тогда
								...
								Если текПараметрЗапроса.Тип = "ТаблицаЗначений" Тогда
									ПрочитатьТаблицуЗначенийИзXML(ФайлXML, текПараметрЗапроса, КоличествоСтрок,
										КоличествоКолонок);
								КонецЕсли;
				...
				//Сизов+
			ИначеЕсли ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ФайлXML.Имя = "Colour" Тогда
				ФайлXML.Прочитать();
				ЦветШрифт = ЗначениеИзСтрокиВнутр(ФайлXML.Значение);
				ФайлXML.Прочитать();
				Продолжить;
			ИначеЕсли ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ФайлXML.Имя = "Decoration" Тогда
				ФайлXML.Прочитать();
				ЦветФонаЗапроса = ЗначениеИзСтрокиВнутр(ФайлXML.Значение);
				ФайлXML.Прочитать();
				Продолжить;
			ИначеЕсли ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ФайлXML.Имя = "Font" Тогда
				ФайлXML.Прочитать();
			...
			Если ФайлXML.ТипУзла = ТипУзлаXML.КонецЭлемента И ФайлXML.Имя = "query" Тогда
				Если СтруктураПланаЗапроса.СКЛЗапрос <> "" И СтруктураПланаЗапроса.ПланВыполненияЗапроса <> ""
					И СтруктураПланаЗапроса.ТипСУБД <> "" Тогда
					текЭлементЗапроса.АдресХранилищаПланаЗапроса = ПоместитьВоВременноеХранилище(СтруктураПланаЗапроса,
						Новый УникальныйИдентификатор);
				Иначе
					текЭлементЗапроса.АдресХранилищаПланаЗапроса = ПоместитьВоВременноеХранилище(Неопределено,
						Новый УникальныйИдентификатор);
				КонецЕсли;
			КонецЕсли;
...
Функция ПроверкаКонцаТэгов(ФайлXML)
	...
	Если (ФайлXML.ТипУзла = ТипУзлаXML.КонецЭлемента И ФайлXML.Имя = "query") Или (ФайлXML.ТипУзла
		= ТипУзлаXML.КонецЭлемента И ФайлXML.Имя = "parameters") Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция ДобавлениеНовогоПараметра(ТекущийПрочитанныйПараметр, ИдентификаторЗапроса)
	...
	ЭлементПараметр = Новый Структура("ИдентификаторЗапроса, Имя, Тип, Значение", ИдентификаторЗапроса,
...
//
Функция ВыполнитьЗапрос(ТекстЗапроса, ПараметрыЗапроса, ТДРезультатаЗапроса, ПараметрыВыводаЗапроса,
	ОтчетПоВыполнениюЗапроса, МеткаЗапроса) Экспорт
	...
	Успешно = ВывестиРезультатЗапросов(ТДРезультатаЗапроса, МассивДанныхПоЗапросу, ПараметрыЗапроса,
		ПараметрыВыводаЗапроса, ОтчетПоВыполнениюЗапроса);
	Если Не Успешно Тогда
		Если ВозможноОшибкаИзЗаТочкиСЗапятой(ТекстЗапроса) Тогда
			ОтчетПоВыполнениюЗапроса.ТекстСообщения = НСтр(
				"ru = 'Результат запроса не был выведен. Возможно не экранирована точка с запятой. Для экранирования точки с запятой используется обратный слеш -""\;""(см. справку)'");
		Иначе
			ОтчетПоВыполнениюЗапроса.ТекстСообщения = НСтр(
				"ru = 'Запрос не был выполнен, т.к. текст запроса некорректный'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьМеткуЗапроса(ТекстЗапроса, Метка, Статус)
	...
	Если Статус = "begin" Тогда
		ТекстЗапроса = "ВЫБРАТЬ ""Marker_" + Метка + "_" + Статус + """ КАК МЕТКА ПОМЕСТИТЬ Marker_begin " + Символы.ПС
			+ ";" + Символы.ПС + ТекстЗапроса + Символы.ПС + ";" + Символы.ПС;
	Иначе
		ТекстЗапроса =  Символы.ПС + ТекстЗапроса + "ВЫБРАТЬ ""Marker_" + Метка + "_" + Статус
			+ """ КАК МЕТКА ПОМЕСТИТЬ Marker_end " + Символы.ПС + ";" + Символы.ПС;
	КонецЕсли;
...
//    * ТекстСообщения - текст сообщения об ошибке.
//
Функция ВывестиРезультатЗапросов(ТДРезультатаЗапроса, МассивДанныхПоЗапросу, ПараметрыЗапроса, ПараметрыВыводаЗапроса,
	...
	Для Индекс = 0 По КоличествоТекстовЗапросов - 1 Цикл
		...
		НакопленныйТекст          = НакапливаемыйЗапрос.Текст + СимволРазделенияЗапросов + МассивТекстов.Получить(
			Индекс);
		...
		Если ПозицияПоместить <> 0 И ПараметрыВыводаЗапроса.ВыводитьВременныеТаблицы Тогда
			...
			Если Результат <> Неопределено Тогда
				ТД = ВывестиРезультатОдногоЗапроса(ИмяЗапроса, Результат, Свертка, ПараметрыВыводаЗапроса, Ложь,
					КоличествоСтрокОдногоЗапроса, МассивШириныКолонок);
		...
		ИначеЕсли ПозицияПоместить = 0 Тогда // Если таблица не временная, то используется готовый результат.
			...
			ТД = ВывестиРезультатОдногоЗапроса(ИмяЗапроса, Результат, Свертка, ПараметрыВыводаЗапроса, Иерархия,
				КоличествоСтрокОдногоЗапроса, МассивШириныКолонок);
			ТДРезультатаЗапроса.Вывести(ТД);
		КонецЕсли;
		ОтчетПоВыполнениюЗапроса.КоличествоСтрок = ОтчетПоВыполнениюЗапроса.КоличествоСтрок
			+ КоличествоСтрокОдногоЗапроса;
	КонецЦикла;
...
//
Функция ВывестиРезультатОдногоЗапроса(ИмяЗапроса, РезультатЗапроса, Открыта, ПараметрыВыводаЗапроса, Иерархия,
	КоличествоСтрок, МассивШириныКолонок)
	...
	Если ПараметрыВыводаЗапроса.ВыводитьРезультатыЗапроса >= 0 И (ПараметрыВыводаЗапроса.КоличествоСтрокВыведено
		< ПараметрыВыводаЗапроса.КоличествоСтрок) Тогда
...
Процедура ВывестиДеталиБезИерархии(ОбщиеДетали, Результат, ПараметрыВыводаЗапроса, Уровень, КоличествоКолонок)
	...
	КоличествоСтрокРезультатаЗапроса = ?(ПараметрыВыводаЗапроса.ВыводитьРезультатыЗапроса = -1,
		ПараметрыВыводаЗапроса.КоличествоСтрок, ПараметрыВыводаЗапроса.ВыводитьРезультатыЗапроса);
...
Процедура ВывестиДеталиСИерархией(ОбщиеДетали, Результат, ПараметрыВыводаЗапроса, Уровень, КоличествоКолонок,
	ИндексСтроки, ТолькоПодсчетКоличестваСтрок = Ложь)
	МакетВывода = ПолучитьМакет("РезультатВыполненияЗапроса");
	Открыта = Истина;
	...
	Если ТолькоПодсчетКоличестваСтрок Тогда
		Для Каждого Подчиненный Из Подчиненные Цикл
			ВывестиДеталиСИерархией(ОбщиеДетали, Подчиненный, ПараметрыВыводаЗапроса, Уровень + 1, КоличествоКолонок,
				ИндексСтроки, Истина);
		КонецЦикла;
		Возврат;
	КонецЕсли;
	...
	КоличествоСтрокРезультатаЗапроса = ?(ПараметрыВыводаЗапроса.ВыводитьРезультатыЗапроса = -1,
		Подчиненные.Количество(), ПараметрыВыводаЗапроса.ВыводитьРезультатыЗапроса);
...
Функция ЭтоСсылка(Тип) Экспорт
	...
	Возврат Справочники.ТипВсеСсылки().СодержитТип(Тип) Или Документы.ТипВсеСсылки().СодержитТип(Тип)
...
Функция ПутьККонфигурационномуФайлу()
	...
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Если Не ((СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Windows_x86) Или (СистемнаяИнформация.ТипПлатформы
...
Процедура ПрочитатьКонфигурациюИзФайла(СтруктураФайлаКонфигурации, ПараметрыФайлаКонфигурации)
	...
	Если ПараметрыФайлаКонфигурации.КаталогКонфигурацииПриложения <> Неопределено Тогда
		...
		Если ПараметрыФайлаКонфигурации.ФайлКонфигурации.Существует() Тогда
			...
			Пока Истина Цикл
				Если ПрочитатьТекстДоТега(ПараметрыТекста,
					СтруктураФайлаКонфигурации.ШаблоныТеговКонсоли.ОткрывающийНачало) Тогда
					...
					Если ПрочитатьТекстДоТега(ПараметрыТекста,
						СтруктураФайлаКонфигурации.ШаблоныТеговКонсоли.Закрывающий, Истина) Тогда
...
Процедура ДобавитьБлокВСтруктуруКонфигурации(СтруктураФайлаКонфигурации, ПараметрыТекста, ЭтоБлокКонсоли)
	...
	Иначе
		...
		Если ЕстьБлокНеКонсоли Тогда
			ИмеющийсяБлок.Текст = ИмеющийсяБлок.Текст + ?(ИмеющийсяБлок.Текст = "", "", Символы.ПС)
...
Функция ПрочитатьТекстДоТега(ПараметрыТекста, ШаблонТега, ВключатьТеги = Ложь)
	...
	Пока ПараметрыТекста.ТекущаяСтрока <> Неопределено Цикл
		...
		Если СтрНайти(НРег(ПараметрыТекста.ТекущаяСтрока), "</config>") > 0 Тогда
			ПараметрыТекста.ТекстБлока = ПараметрыТекста.ТекстБлока + ?(ПараметрыТекста.ТекстБлока = "", "",
		...
		ПараметрыТекста.ТекстБлока = ПараметрыТекста.ТекстБлока + ?(ПараметрыТекста.ТекстБлока = "", "", Символы.ПС)
...
Функция ЗаписатьКонфигурациюВФайл(СтруктураФайлаКонфигурации, ПараметрыФайлаКонфигурации, РезультатВключения, Записывать)
	...
	Если Записывать Тогда
		...
		Для Каждого БлокКонфигурации Из СтруктураФайлаКонфигурации.БлокиКонфигурации Цикл
			...
			Если БлокКонфигурации.ЭтоБлокКонсоли И БлокКонфигурации.Удалить Тогда
			...
			Если Не БлокКонфигурации.Удалить И Не ЕстьPlanSql И СтрНайти(БлокКонфигурации.Текст, "<plansql/>") > 0 Тогда
				ЕстьPlanSql = Истина;
			КонецЕсли;

		КонецЦикла;

		Если Не ЕстьPlanSql И СтруктураФайлаКонфигурации.КоличествоБлоковКонсоли > 0 Тогда
			БлокPlanSql = СтруктураФайлаКонфигурации.ШаблоныТеговКонсоли.ОткрывающийНачало + " -->" + Символы.ПС
				+ "<plansql/>" + Символы.ПС + СтруктураФайлаКонфигурации.ШаблоныТеговКонсоли.Закрывающий + " -->";
			ТекстФайлаКонфигурации = ТекстФайлаКонфигурации + ?(ТекстФайлаКонфигурации = "", "", Символы.ПС)
				+ БлокPlanSql;
		КонецЕсли;

		Если Не ПустаяСтрока(ТекстФайлаКонфигурации) Тогда
			ТекстФайлаКонфигурации = СтрЗаменить(ТекстБлокаНеКонсоли, "</config>", ТекстФайлаКонфигурации + Символы.ПС
				+ "</config>");
		Иначе
		...
		Исключение
			...
			РезультатВключения.Вставить("Причина", НСтр("ru = 'Ошибка записи конфигурационного файла в каталоге'") + " "
				+ ПараметрыФайлаКонфигурации.КаталогКонфигурацииПриложения + Символы.ПС + НСтр(
...
Процедура ИзменитьКонфигурациюВключениеТЖ(СтруктураФайлаКонфигурации, ПараметрыФайлаКонфигурации)
	...
	Если ПараметрыФайлаКонфигурации.КаталогКонфигурацииПриложения <> Неопределено
		...
		Если БлокКонфигурацииТекущегоСеанса = Неопределено Тогда
			...
			БлокКонфигурацииТекущегоСеанса = БлокКонфигурацииСеанса(ФильтрТекущегоСеанса,
				СтруктураФайлаКонфигурации.ШаблоныТеговКонсоли);
		...
		Иначе
			...
			БлокКонфигурацииТекущегоСеанса.Текст = СтрЗаменить(БлокКонфигурацииТекущегоСеанса.Текст,
				"<?xml version=""1.0""?>", Тэг);
		...
		Если ИнформационнаяБазаФайловая Тогда
			Если ВерсияПриложенияЧисло(ВерсияПлатформы) >= ВерсияПриложенияЧисло("8.3.14") Тогда
				...
				ИначеЕсли ЗначениеСобытия = "DBV8DBEng" Тогда
					ТекстФильтров = "  <eq property=""DataBase"" value=""" + ФильтрТекущегоСеанса.DataBase + """/>"
						+ Символы.ПС;
			...
			КонецЕсли;
		Иначе
			ТекстФильтров = "  <eq property=""p:processName"" value=""" + ФильтрТекущегоСеанса.processname + """/>"
				+ Символы.ПС + "  <eq property=""usr"" value=""" + ФильтрТекущегоСеанса.usr + """/>" + Символы.ПС
				+ "  <eq property=""sessionid"" value=""" + ФильтрТекущегоСеанса.sessionid + """/>" + Символы.ПС;
		КонецЕсли;

		Текст = Текст + "<event>" + Символы.ПС + "  <eq property=""name"" value=""" + ЗначениеСобытия + """/>"
			+ Символы.ПС + ТекстФильтров + "</event>" + Символы.ПС;
	КонецЦикла;
	...
	Если ПараметрыФайлаКонфигурации.КаталогКонфигурацииПриложения <> Неопределено
		...
		Для Каждого БлокКонфигурации Из СтруктураФайлаКонфигурации.БлокиКонфигурации Цикл
			...
			Если БлокПросрочен Тогда
				...
				СтруктураФайлаКонфигурации.КоличествоБлоковКонсоли = СтруктураФайлаКонфигурации.КоличествоБлоковКонсоли
					- 1;
				Продолжить;
			КонецЕсли;
		...
		Если ДобавлятьВМассив Тогда
			Если ДобавлятьВМассив И СтрНайти(Строка, ИДМетки + "_end") = 0 Тогда
				Если Сред(Строка, 3, 1) = ":" И Сред(Строка, 6, 1) = "." Тогда
					Если ЗначениеЗаполнено(ВременнаяСтрока) И СтрНайти(ВременнаяСтрока, "Sql=") > 0 И СтрНайти(
						ВременнаяСтрока, "planSQLText=") > 0 И СтрНайти(ВременнаяСтрока, "Marker_" + ИДМетки) = 0 Тогда
...
Функция ИдентификаторПроцессаОС()
	...
	Если ИДТекущегоПроцесса = Неопределено Тогда
		...
		ИДТекущегоПроцесса = ПолучитьCOMОбъект(
			"winmgmts:{impersonationLevel=impersonate}!\\.\root\CIMV2:Win32_Process.Handle='" + Формат(
			Процесс.ProcessID, "ЧГ=0") + "'").ParentProcessID;
		Процесс.Terminate();
	КонецЕсли;
	...
	Для Каждого Узел Из ТекущийУзел.ДочерниеУзлы Цикл
		...
		Если Узел.ИмяУзла = "log" Тогда
			...
			Если ПараметрыОбработки.ФильтрыСобытияТЖ.ТекущееСобытие = Неопределено
				...
				И ПараметрыОбработки.ФильтрыСобытияТЖ.sessionid = Неопределено И ПараметрыОбработки.ЗаменитьФильтры Тогда
				МассивСобытий = МассивСобытийКонсолиЗапросов(ИнформационнаяБазаФайловая);
			...
			ИначеЕсли ИнформационнаяБазаФайловая И ВерсияПриложенияЧисло(ВерсияПлатформы) >= ВерсияПриложенияЧисло(
				"8.3.14") И ПараметрыОбработки.ФильтрыСобытияТЖ.sessionid = Неопределено
				...
				ПараметрыОбработки.ТекущееСостояниеТЖ.Вставить("Включен", Истина);
				ПараметрыОбработки.ТекущееСостояниеТЖ.Вставить("КаталогСЛогФайлами", Узел.ПолучитьАтрибут("location"));
			ИначеЕсли ИнформационнаяБазаФайловая И ПараметрыОбработки.ФильтрыСобытияТЖ.ТекущееСобытие <> Неопределено
				...
				ПараметрыОбработки.ТекущееСостояниеТЖ.Вставить("Включен", Истина);
				ПараметрыОбработки.ТекущееСостояниеТЖ.Вставить("КаталогСЛогФайлами", Узел.ПолучитьАтрибут("location"));
			ИначеЕсли Не ИнформационнаяБазаФайловая И ПараметрыОбработки.ФильтрыСобытияТЖ.ТекущееСобытие <> Неопределено
		...
		Если Узел.ИмяУзла = "event" Тогда
			...
			Если ИнформационнаяБазаФайловая И ВерсияПриложенияЧисло(ВерсияПлатформы) >= ВерсияПриложенияЧисло("8.3.14")
		...
		Если Узел.ИмяУзла = "eq" Тогда
			...
			Если ИнформационнаяБазаФайловая И ВерсияПриложенияЧисло(ВерсияПлатформы) >= ВерсияПриложенияЧисло("8.3.14") Тогда
...
Процедура НайтиУзлыКУдалению(ТекущийУзел, ФильтрТекущегоСеанса, УзлыКУдалению, КоличествоEvent = 0,
	ФильтрыСобытияТЖ = Неопределено)
	...
	Для Каждого Узел Из ТекущийУзел.ДочерниеУзлы Цикл
		...
		Если Узел.ИмяУзла = "event" Тогда
			...
			Если ИнформационнаяБазаФайловая И ВерсияПриложенияЧисло(ВерсияПлатформы) >= ВерсияПриложенияЧисло("8.3.14")
				И ФильтрыСобытияТЖ.ТекущееСобытие <> Неопределено И ФильтрыСобытияТЖ.sessionid = Неопределено
				И ФильтрыСобытияТЖ.ТекущаяБаза = ФильтрТекущегоСеанса.DataBase И ФильтрыСобытияТЖ.ТекущийПользователь
				= Неопределено Тогда
				УзлыКУдалению.Добавить(Узел);
			ИначеЕсли ИнформационнаяБазаФайловая И ФильтрыСобытияТЖ.ТекущееСобытие <> Неопределено
				И ФильтрыСобытияТЖ.sessionid = Неопределено И ФильтрыСобытияТЖ.ТекущаяБаза = Неопределено
				И ФильтрыСобытияТЖ.ТекущийПользователь = Неопределено Тогда
				УзлыКУдалению.Добавить(Узел);
			ИначеЕсли Не ИнформационнаяБазаФайловая И ФильтрыСобытияТЖ.ТекущееСобытие <> Неопределено
				И ФильтрыСобытияТЖ.sessionid = ФильтрТекущегоСеанса.sessionid И ФильтрыСобытияТЖ.ТекущаяБаза
				= ФильтрТекущегоСеанса.processname И ФильтрыСобытияТЖ.ТекущийПользователь = ФильтрТекущегоСеанса.usr Тогда
				УзлыКУдалению.Добавить(Узел);
			ИначеЕсли Не ИнформационнаяБазаФайловая И ФильтрТекущегоСеанса.usr = ФильтрыСобытияТЖ.ТекущийПользователь
...
Функция СеансКонсолиЗапросовНеактивен(ФильтрТекущегоСеанса, ФильтрыСобытияТЖ)
	...
	Для Каждого СеансТекущейБазы Из СеансыТекущейБазы Цикл
		Если СеансТекущейБазы.Пользователь.Имя = ФильтрыСобытияТЖ.ТекущийПользователь И Строка(
			СеансТекущейБазы.НомерСеанса) = ФильтрыСобытияТЖ.sessionid Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
	...
	Если ПараметрыФайлаКонфигурации.КаталогКонфигурацииПриложения <> Неопределено
		...
		Если БлокКонфигурацииТекущегоСеанса <> Неопределено Тогда
			...
			Если ТекущееСостояниеТЖ <> Неопределено Тогда
				...
				ПараметрыТехнологическогоЖурнала.ИдентификаторПроцессаОС = Формат(ИдентификаторПроцессаОС,
					"ЧРД=; ЧРГ=; ЧГ=0");
			КонецЕсли;
...
// 
Процедура ДеревоПланаВыполненияЗапроса(Знач ПланЗапросаТекст, Знач ПланЗапросаМетаданные, ДеревоПланЗапроса,
	СуммарнаяСтоимостьОбщая) Экспорт
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	//	Название, описание и версия обработки для администратора - используется при подключении обработки
	ПараметрыРегистрации.Вставить("Наименование", "Консоль запросов 83");
	ПараметрыРегистрации.Вставить("Информация", "Консоль запросов 83");
	ПараметрыРегистрации.Вставить("Версия", "1.0");  
	
	//	Минимальная версия БСП, поддерживаемую это обработкой. Должна быть не ниже 1.2.1.4
	ПараметрыРегистрации.Вставить("ВерсияБСП", "1.2.1.4");
	
	//	Определяет исполнение кода в безопасном режиме.
	//	При исполнении в безопасном режиме часть функционала может быть недоступна для пользователей без полных прав.
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	
	//	Заполнение данных о командах в интерфейсе
	ТаблицаКоманд = СформироватьСтруктуруТаблицыКоманд();

	НоваяКоманда = ТаблицаКоманд.Добавить();
	//	Представление команды для пользователя - в интерфейсе
	НоваяКоманда.Представление = "Консоль запросов 83"; 
	
	//	Идентификатор - строка уникальная в рамках этой обработки
	НоваяКоманда.Идентификатор = "bt_КонсольЗапросов83";
	
	//	Опредляет вариант запуска команды.
	//	Доступные значения - ОткрытиеФормы, ВызовКлиентскогоМетода, ВызовСерверногоМетода, ЗаполнениеФормы
	НоваяКоманда.Использование = "ОткрытиеФормы";
	
	//	Опредялет нужно ли показывать оповещение при начале и окончании работы обработки
	//НоваяКоманда.ПоказыватьОповещение = Истина;
	
	//	Используется для внешних печатных форм при печати на основе макета. Должен содержать значение "ПечатьMXL"
	//НоваяКоманда.Модификатор = "ПечатьMXL";

	//	Содержит список идентификаторов команд печати (через запятую), которые должн замениться текущей командой
	//НоваяКоманда.ЗаменяемыеКоманды = "";
...
Функция СформироватьСтруктуруТаблицыКоманд()
